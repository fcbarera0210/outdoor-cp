...
export default function AdminRutasPage() {
  return (
    <div>
      ...
      >
        {rutas.map((ruta) => (
          <tr key={ruta.slug} className="border-t border-gray-200 hover:bg-gray-50">
            <td className="px-4 py-3 font-medium text-brand-dark">{ruta.nombre}</td>
            <td className="px-4 py-3 text-gray-600">{ruta.zona}</td>
            <td className="px-4 py-3">
              <span
                className={`px-2 py-1 rounded text-xs font-medium ${dificultadColors[ruta.dificultad]}`}
              >
                {ruta.dificultad}
              </span>
            </td>
            <td className="px-4 py-3 text-gray-600">{ruta.duracion}</td>
            <td className="px-4 py-3">
